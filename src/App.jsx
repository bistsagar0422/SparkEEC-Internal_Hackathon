...
import { sx, merge, HoverBox, useCountdown } from './lib.jsx';
import {
...
function Nav({ page, go, theme, toggleTheme }) {
  ...
  return (
    <div style={sx('position:fixed;top:0;left:0;right:0;z-index:100;background:var(--nav-bg);backdrop-filter:blur(14px);border-bottom:1px solid rgba(var(--wc),0.07)')}>
      <div style={sx('max-width:1200px;margin:0 auto;padding:0 28px;height:76px;display:flex;align-items:center;justify-content:space-between;gap:24px')}>
        <div onClick={() => go('home')} style={sx('cursor:pointer;user-select:none;display:flex;align-items:center;font-size:23px;font-weight:700;letter-spacing:-0.02em')}>
          <LogoMark size={42} radius={11} style="margin-right:13px" />
          {letters.map((ch, i) => (
        ...
        <div style={sx('display:flex;align-items:center;gap:12px')}>
          ...
          <HoverBox
            as="button"
            onClick={() => go('register')}
            base="cursor:pointer;background:transparent;border:1px solid rgba(240,86,159,0.55);color:#f77fb8;border-radius:999px;padding:11px 24px;font-size:15.5px;font-weight:600;white-space:nowrap;transition:all 0.25s"
            hover="background:rgba(240,86,159,0.12);color:var(--fg-strong);border-color:#f0569f"
          >
            Register
          </HoverBox>
        </div>
      </div>
      <div style={sx('position:absolute;left:0;right:0;bottom:-1px;height:2px;background:linear-gradient(90deg,#f0569f,#a06bf5,#5f8bff);transform-origin:left;transform:scaleX(0);animation:growX linear both;animation-timeline:scroll()')} />
    </div>
  );
}

/* ============================ SECTION LABEL ============================ */
const Kicker = ({ children, color = '#f77fb8' }) => (
  ...
  return (
    <div>
      ...
      <div style={sx('position:relative;overflow:hidden;padding:170px 28px 84px;text-align:center')}>
        ...
        <div style={sx('position:absolute;top:120px;left:-160px;width:520px;height:520px;background:radial-gradient(closest-side,rgba(240,86,159,0.14),transparent);pointer-events:none;animation:orbDrift 11s ease-in-out infinite')} />
        ...
        <div style={sx('position:relative;max-width:1020px;margin:0 auto;display:flex;flex-direction:column;align-items:center;gap:22px')}>
          <div style={sx('display:inline-flex;align-items:center;gap:10px;border:1px solid rgba(var(--wc),0.14);background:rgba(var(--wc),0.04);border-radius:999px;padding:8px 18px;animation:fadeUp 0.7s cubic-bezier(0.22,1,0.36,1) both')}>
            <span style={sx('width:7px;height:7px;border-radius:50%;background:#f0569f;animation:pulseDot 2s ease-out infinite')} />
          ...
          <h1 style={sx('margin:0;font-size:clamp(60px,10.5vw,138px);line-height:0.98;font-weight:700;letter-spacing:-0.035em;color:var(--fg-strong);animation:fadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.1s both')}>
            SPARK
            <span style={sx('background:linear-gradient(96deg,#f0569f 0%,#a06bf5 45%,#5f8bff 70%,#f0569f 100%);background-size:200% auto;-webkit-background-clip:text;background-clip:text;color:transparent;animation:shimmer 5s linear infinite')}>EEC 1.0</span>
          </h1>

          <div style={sx('font-size:clamp(17px,2.1vw,22px);color:var(--fg);font-weight:500;animation:fadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.2s both')}>17th – 18th July 2026 · <span style={sx('color:#f77fb8')}>27 Hours of Pure Creation</span> · Sanepa-2, Lalitpur</div>
          ...
          {!(cd.live || cd.over) && (
            <div style={sx('display:flex;gap:14px;margin-top:14px;flex-wrap:wrap;justify-content:center;animation:fadeUp 0.8s cubic-bezier(0.22,1,0.36,1) 0.4s both')}>
              ...
              <HoverBox base="width:104px;background:rgba(240,86,159,0.08);border:1px solid rgba(240,86,159,0.35);border-radius:14px;padding:16px 0 12px;box-shadow:var(--card-shadow);transition:transform 0.32s var(--ease-out),border-color 0.3s,box-shadow 0.32s ease" hover="transform:translateY(-4px);border-color:rgba(240,86,159,0.6);box-shadow:var(--card-shadow-hover)">
                <div style={sx("font-family:'JetBrains Mono',monospace;font-size:38px;font-weight:600;color:#f77fb8;animation:blink 1s steps(1) infinite")}>{cd.secs}</div>
          ...
          {(cd.live || cd.over) && (
            <div style={sx("font-family:'JetBrains Mono',monospace;font-size:15px;letter-spacing:0.24em;color:#f77fb8;text-transform:uppercase;margin-top:10px")}>
        ...
        <div style={sx('max-width:1160px;margin:0 auto;padding:34px 28px;display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:24px;text-align:center')}>
          {STATS.map(([value, label]) => (
            <div key={label} style={sx('animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 40%')}>
              <div style={sx('font-size:40px;font-weight:700;background:linear-gradient(96deg,#f0569f,#a06bf5,#5f8bff);-webkit-background-clip:text;background-clip:text;color:transparent')}>{value}</div>
      ...
      {/* ABOUT */}
      <div style={sx('max-width:1160px;margin:0 auto;padding:96px 28px;display:grid;grid-template-columns:1.25fr 1fr;gap:56px;align-items:start')}>
        ...
        <div style={sx('max-width:1160px;margin:0 auto;padding:96px 28px;text-align:center')}>
          <div style={sx('animation:fadeUp 0.9s ease both;animation-timeline:view();animation-range:entry 0% entry 30%')}>
            ...
            <h2 style={sx('margin:16px auto 42px;max-width:860px;font-size:clamp(26px,3.4vw,38px);font-weight:700;line-height:1.25;letter-spacing:-0.015em')}>
              “Innovate for Impact: Harnessing <span style={sx('background:linear-gradient(96deg,#f0569f,#a06bf5);-webkit-background-clip:text;background-clip:text;color:transparent')}>Artificial Intelligence</span> and <span style={sx('background:linear-gradient(96deg,#a06bf5,#5f8bff);-webkit-background-clip:text;background-clip:text;color:transparent')}>Emerging Technologies</span> for Sustainable Development.”
            </h2>
          </div>
          <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;text-align:left')}>
            {[
              ['01 · REAL PROBLEM', '#f77fb8', 'rgba(240,86,159,0.45)', 'A defined user who hurts today', 'Anchor your build in evidence — who hurts, how much, why now.'],
      ...
      <div style={sx('border-top:1px solid rgba(var(--wc),0.07);background:rgba(var(--wc),0.015)')}>
        <div style={sx('max-width:1200px;margin:0 auto;padding:120px 28px')}>
          ...
          <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:28px')}>
            {[
              ['#f77fb8', 'Lead organizer', 'SOCISE-EEC', 'Society of Computer, IT and Software Engineering', sociseLogo],
              ['#c5a3fa', 'In collaboration with', 'AWS Student Builder Group – EEC', 'Cloud & builder community chapter', awsLogo],
              ['#8fb0ff', 'In collaboration with', 'IEEE EEC Student Branch', 'IEEE student community at EEC', ieeeLogo],
            ].map(([fg, kicker, name, desc, logo], i) => (
              <div key={i} className="org-card" style={sx(`animation:fadeUp 0.8s ease ${0.1 * i}s both;animation-timeline:view();animation-range:entry 0% entry 30%`)}>
                ...
                <div style={sx(`font-family:'JetBrains Mono',monospace;font-size:12px;letter-spacing:0.2em;color:${fg};text-transform:uppercase;margin-bottom:12px`)}>{kicker}</div>
      ...
      <div style={sx('max-width:1200px;margin:0 auto;padding:120px 28px')}>
        ...
        <div style={sx('max-width:720px;margin:0 auto 40px;display:grid;grid-template-columns:repeat(auto-fit,minmax(240px,1fr));gap:28px')}>
          {[
            ['Anish Bhattarai', 'Advisor', '#f77fb8',
             'linear-gradient(160deg,rgba(240,86,159,0.1),rgba(160,107,245,0.09) 55%,rgba(95,139,255,0.1))', 'rgba(240,86,159,0.35)', '0s'],
            ['Sagar Bist', 'Event Lead', '#c5a3fa',
             'linear-gradient(160deg,rgba(95,139,255,0.1),rgba(160,107,245,0.09) 55%,rgba(240,86,159,0.1))', 'rgba(160,107,245,0.35)', '0.1s'],
        ...
        <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:22px')}>
          {TEAM.map(([name, role], i) => {
            const photo = MEMBER_PHOTOS[name];
            const fg = ['#f77fb8', '#c5a3fa', '#8fb0ff'][i % 3];
            return (
      ...
      <div style={sx('max-width:1160px;margin:0 auto;padding:96px 28px 96px')}>
        <div style={sx('position:relative;overflow:hidden;border-radius:22px;border:1px solid rgba(160,107,245,0.35);background:linear-gradient(140deg,rgba(240,86,159,0.12),rgba(160,107,245,0.1) 50%,rgba(95,139,255,0.12));padding:64px 32px;text-align:center;animation:fadeUp 0.9s ease both;animation-timeline:view();animation-range:entry 0% entry 30%')}>
          ...
          <HoverBox as="button" onClick={() => go('register')} base="position:relative;cursor:pointer;border:none;border-radius:999px;padding:16px 38px;font-size:16px;font-weight:600;color:#ffffff;background:linear-gradient(96deg,#f0569f,#a06bf5,#5f8bff);box-shadow:0 10px 34px rgba(160,107,245,0.4);transition:transform 0.25s,box-shadow 0.25s" hover="transform:translateY(-3px);box-shadow:0 16px 48px rgba(160,107,245,0.6)">Register your team →</HoverBox>
        </div>
      </div>
    </div>
  );
...
function Tracks() {
  ...
  return (
    <div style={sx('max-width:1160px;margin:0 auto;padding:150px 28px 96px;animation:fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both')}>
      ...
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(420px,1fr));gap:18px')}>
            ...
            <HoverBox key={title} onClick={() => setOpen(isOpen ? -1 : i)} base={`cursor:pointer;background:rgba(var(--wc),0.035);border:1px solid ${isOpen ? 'rgba(160,107,245,0.55)' : 'rgba(var(--wc),0.09)'};border-radius:16px;padding:26px 28px;box-shadow:var(--card-shadow);transition:transform 0.32s var(--ease-out),border-color 0.3s,box-shadow 0.32s ease;animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 25%`} hover="transform:translateY(-3px);border-color:rgba(160,107,245,0.5);box-shadow:var(--card-shadow-hover)">
              <div style={sx('display:flex;align-items:center;justify-content:space-between;gap:16px')}>
                <div style={sx('display:flex;align-items:baseline;gap:16px;min-width:0')}>
                  <span style={sx("font-family:'JetBrains Mono',monospace;font-size:13px;color:#f77fb8")}>{String(i + 1).padStart(2, '0')}</span>
    ...
    <div style={sx('max-width:960px;margin:0 auto;padding:150px 28px 96px;animation:fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both')}>
      ...
      <div style={sx('display:flex;gap:12px;margin-bottom:36px;flex-wrap:wrap')}>
        {tabs.map((d) => {
          const active = day === d;
          return (
            <button key={d} onClick={() => setDay(d)} style={sx(`cursor:pointer;border:1px solid ${active ? 'rgba(240,86,159,0.5)' : 'rgba(var(--wc),0.14)'};background:${active ? 'linear-gradient(96deg,rgba(240,86,159,0.25),rgba(160,107,245,0.25))' : 'rgba(var(--wc),0.04)'};color:${active ? 'var(--fg-strong)' : 'var(--muted)'};border-radius:999px;padding:13px 28px;font-size:15px;font-weight:600;transition:all 0.25s`)}>
      ...
      <div>
        {rows.map(([time, activity, tags, big], i) => (
          <HoverBox key={i} base={`display:grid;grid-template-columns:150px 1fr;gap:24px;padding:17px 4px;border-bottom:1px solid rgba(var(--wc),0.06);align-items:baseline;transition:background 0.2s;animation:fadeUp 0.55s ease both;animation-delay:${(Math.min(i, 12) * 0.04).toFixed(2)}s`} hover="background:rgba(var(--wc),0.025)">
            <div style={sx(`font-family:'JetBrains Mono',monospace;font-size:13.5px;color:${big ? '#f77fb8' : 'var(--muted)'};white-space:nowrap`)}>{time}</div>
      ...
      <div style={sx('margin-top:32px;background:rgba(240,86,159,0.06);border:1px solid rgba(240,86,159,0.25);border-radius:14px;padding:20px 24px;animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 40%')}>
        <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.18em;color:#f77fb8;text-transform:uppercase;margin-bottom:8px")}>Checkpoint rule</div>
  ...
  return (
    <div style={sx('max-width:1160px;margin:0 auto;padding:150px 28px 96px;animation:fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both')}>
      ...
      <div style={sx('max-width:560px;margin:0 auto 20px')}>
        <HoverBox base="position:relative;background:linear-gradient(160deg,rgba(240,86,159,0.13),rgba(160,107,245,0.11) 55%,rgba(95,139,255,0.13));border:1px solid rgba(240,86,159,0.45);border-radius:18px;padding:40px 32px;text-align:center;display:flex;flex-direction:column;gap:8px;box-shadow:var(--card-shadow-hover);transition:transform 0.32s var(--ease-out),box-shadow 0.32s ease;animation:fadeUp 0.7s ease both" hover="transform:translateY(-6px);box-shadow:var(--card-shadow-hover)">
          <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.22em;color:#f77fb8;text-transform:uppercase")}>🏆 Champion</div>
          <div style={sx('font-size:52px;font-weight:700;background:linear-gradient(96deg,#f0569f,#a06bf5,#5f8bff,#f0569f);background-size:200% auto;-webkit-background-clip:text;background-clip:text;color:transparent;animation:shimmer 5s linear infinite')}>NPR 10,000</div>
          <div style={sx('font-size:15.5px;line-height:1.6;color:var(--muted2)')}>Trophy · certificates · mentorship &amp; incubation support</div>
        </HoverBox>
      </div>

      {/* RUNNERS-UP */}
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:20px;align-items:stretch;margin-bottom:20px')}>
        <HoverBox base="background:rgba(240,86,159,0.08);border:1px solid rgba(240,86,159,0.3);border-radius:18px;padding:32px;text-align:center;display:flex;flex-direction:column;gap:8px;box-shadow:var(--card-shadow);transition:transform 0.32s var(--ease-out),border-color 0.3s,box-shadow 0.32s ease;animation:fadeUp 0.7s ease 0.1s both" hover="transform:translateY(-4px);border-color:rgba(240,86,159,0.55);box-shadow:var(--card-shadow-hover)">
          <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.2em;color:#f77fb8;text-transform:uppercase")}>1st Runner-up</div>
      ...
      {/* CATEGORY WINNERS */}
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(260px,1fr));gap:20px;margin-bottom:16px')}>
        {[
          ['rgba(160,107,245', 'Best AI Solution', 'Category trophy & certificates', '#c5a3fa', 'NPR 2,000', '0s'],
          ['rgba(240,86,159', 'Best Innovation', 'Category trophy & certificates', '#c5a3fa', 'NPR 2,000', '0.1s'],
      ...
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:20px;margin-bottom:36px')}>
        {PANELS.map((p) => (
      ...
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:18px')}>
        <div style={sx('background:rgba(var(--wc),0.03);border:1px solid rgba(var(--wc),0.08);box-shadow:var(--card-shadow);border-radius:14px;padding:22px 26px;animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 40%')}>
          <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.18em;color:#f77fb8;text-transform:uppercase;margin-bottom:8px")}>Two-stage pitching</div>
...
function Rules() {
  ...
  return (
    <div style={sx('max-width:1160px;margin:0 auto;padding:150px 28px 96px;animation:fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both')}>
      ...
      <div style={sx('display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:18px;margin-bottom:88px')}>
        {RULE_CARDS.map(([ref, title, body]) => (
          <HoverBox key={ref} base="background:rgba(var(--wc),0.035);border:1px solid rgba(var(--wc),0.09);box-shadow:var(--card-shadow);border-radius:16px;padding:26px 28px;box-shadow:var(--card-shadow);transition:transform 0.32s var(--ease-out),border-color 0.3s,box-shadow 0.32s ease;animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 25%" hover="transform:translateY(-3px);border-color:rgba(160,107,245,0.4);box-shadow:var(--card-shadow-hover)">
            <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.14em;color:#f77fb8;margin-bottom:10px")}>{ref}</div>
            <div style={sx('font-size:17px;font-weight:600;margin-bottom:8px')}>{title}</div>
...
function Register() {
  ...
  return (
    <div style={sx('max-width:1160px;margin:0 auto;padding:150px 28px 96px;animation:fadeUp 0.6s cubic-bezier(0.22,1,0.36,1) both')}>
      ...
      <div style={sx('display:grid;grid-template-columns:1.3fr 1fr;gap:24px;align-items:start')}>
        <div style={sx('display:flex;flex-direction:column;gap:14px')}>
          {REG_STEPS.map(([title, body], i) => (
            <HoverBox key={i} base={`background:rgba(var(--wc),0.035);border:1px solid rgba(var(--wc),0.09);box-shadow:var(--card-shadow);border-radius:16px;padding:24px 28px;display:flex;gap:20px;align-items:flex-start;box-shadow:var(--card-shadow);transition:transform 0.32s var(--ease-out),border-color 0.3s,box-shadow 0.32s ease;animation:fadeUp 0.7s ease both;animation-delay:${(i * 0.1).toFixed(1)}s`} hover="transform:translateY(-3px);border-color:rgba(160,107,245,0.4);box-shadow:var(--card-shadow-hover)">
              <div style={sx("flex:none;width:40px;height:40px;border-radius:50%;background:linear-gradient(140deg,rgba(240,86,159,0.25),rgba(160,107,245,0.25));border:1px solid rgba(240,86,159,0.4);display:flex;align-items:center;justify-content:center;font-family:'JetBrains Mono',monospace;font-size:15px;font-weight:600;color:#f77fb8")}>{i + 1}</div>
              <div>
          ...
          <div style={sx('background:rgba(var(--wc),0.03);border:1px solid rgba(var(--wc),0.08);box-shadow:var(--card-shadow);border-radius:16px;padding:24px 28px;animation:fadeUp 0.8s ease both;animation-timeline:view();animation-range:entry 0% entry 30%')}>
            ...
            <div style={sx('display:flex;flex-direction:column;gap:10px;font-size:16px;line-height:1.6;color:var(--muted2)')}>
              {['Currently enrolled EEC student — any program or year', 'Valid EEC student ID at onboarding', 'Member of one team only — teams of 2, 3, or 4', 'Not an organizer, volunteer, mentor, or judge'].map((t) => (
                <div key={t} style={sx('display:flex;gap:12px')}><span style={sx('color:#f77fb8')}>✓</span><span>{t}</span></div>
              ))}
            </div>
        ...
        <div style={sx('position:sticky;top:92px;display:flex;flex-direction:column;gap:18px')}>
          <div style={sx('background:linear-gradient(160deg,rgba(240,86,159,0.13),rgba(160,107,245,0.11) 55%,rgba(95,139,255,0.13));border:1px solid rgba(240,86,159,0.4);border-radius:18px;padding:34px 32px;text-align:center;box-shadow:var(--card-shadow);animation:fadeUp 0.7s ease 0.15s both')}>
            <div style={sx("font-family:'JetBrains Mono',monospace;font-size:11px;letter-spacing:0.2em;color:#f77fb8;text-transform:uppercase;margin-bottom:10px")}>Registration fee</div>
            ...
            <HoverBox as="a" href={url || '#'} base="display:block;margin-top:24px;text-decoration:none;border:none;border-radius:999px;padding:16px 20px;font-size:16px;font-weight:600;color:#ffffff;background:linear-gradient(96deg,#f0569f,#a06bf5,#5f8bff);box-shadow:0 10px 34px rgba(160,107,245,0.4);transition:transform 0.25s,box-shadow 0.25s" hover="transform:translateY(-3px);box-shadow:0 16px 48px rgba(160,107,245,0.6)">
        ...
        <div style={sx('display:flex;justify-content:space-between;gap:40px;flex-wrap:wrap')}>
          <div style={sx('max-width:360px')}>
            <div style={sx('display:flex;align-items:baseline;gap:8px;margin-bottom:12px')}>
              <span style={sx('font-size:20px;font-weight:700;letter-spacing:-0.02em;color:var(--fg-strong)')}>Spark<span style={sx('background:linear-gradient(96deg,#f0569f,#a06bf5,#5f8bff,#f0569f);background-size:200% auto;-webkit-background-clip:text;background-clip:text;color:transparent;animation:shimmer 5s linear infinite')}>EEC</span></span>
